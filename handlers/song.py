...
async def a(client, message: Message):
    ...
    for i in message.command[1:]:
        query += ' ' + str(i)
    print(query)
    m = await message.reply(f"**{Bn} :-** 🔍 Aranıyor... for {query}")
    ...
    try:
        ...
        try:
            ...
            m.edit(f"**{Bn} :-** 😕 Hiçbir şey bulunamadı. Yazımı biraz değiştirmeyi dene..\n\n{e}")
            return
    except Exception as e:
        m.edit(
           f"**{Bn} :-** 😕 Hiçbir Şey Bulunamadı. Üzgünüm.\n\nBaşka bir anahtarlık deneyin veya belki düzgün heceleyin."
        )
        print(str(e))
        return
    await m.edit(f"**{Bn} :-** 📥 İndiriliyor...\n**Query :-** {query}")
    try:
